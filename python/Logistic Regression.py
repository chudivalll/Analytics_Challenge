...
df = pandas.DataFrame(
    pandasql.sqldf(
        """SELECT CHANNEL, PROD_TYP, SERIES, NumberRange,
                                        UNIT_PRICE, SCHOOL_TYPE, StateID
                                        FROM df
                                        JOIN States
                                        ON States.STATE = df.STATE""",
        locals(),
    )
)
# %%
...
actualstate = pandas.DataFrame(
    pandasql.sqldf(
        "SELECT StateID FROM States WHERE STATE = " + "'" + str(vSTATE) + "'", locals()
    )
)
...
Z = pandas.DataFrame(
    numpy.array(
        [
            [vPROD_TYP, vSERIES, vNumberRange, vPrice, vschooltype, vSTATE],
        ]
    ),
    columns=[
        "PROD_TYP",
...
print(
    "There is a "
    + str(p_prediction.iloc[0, 0] * 100)
    + "% chance this resembles a book that would be sold in Channel 1"
)
print("")
if p_prediction.iloc[0, 1] > p_prediction.iloc[0, 0]:
    print(
        "Therefore the Scholastic algorithm suggests that you use Channel 2 as your means of distribution"
    )
else:
    print(
        "Therefore the Scholastic algorithm suggests that you use Channel 1 as your means of distribution"
    )
# %%
